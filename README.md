...
```console
docker run --name aspnetcore_sample --rm -it -p 8000:80 mcr.microsoft.com/dotnet/core/samples:Tanyo.API
```
...
docker run --name aspnetcore_sample --rm -it -p 8000:80 Tanyo.API
```

You should see the following console output as the application starts.

```console
C:\git\dotnet-docker\samples\Tanyo.API>docker run --name aspnetcore_sample --rm -it -p 8000:80 Tanyo.API
...
cd samples
cd Tanyo.API
docker build --pull -t Tanyo.API -f Dockerfile.alpine-x64 .
docker run --name aspnetcore_sample --rm -it -p 8000:80 Tanyo.API
```
