...
```console
docker run --rm -it -p 8000:80 -v c:\git\dotnet-docker\samples\Tanyo.API:/app/ -w /app/Tanyo.API -e ASPNETCORE_URLS=http://+:80 mcr.microsoft.com/dotnet/core/sdk:3.1 dotnet watch run
```
...
```console
docker run --rm -it -p 8000:80 -v ~/git/dotnet-docker/samples/Tanyo.API:/app/ -w /app/Tanyo.API -e ASPNETCORE_URLS=http://+:80 mcr.microsoft.com/dotnet/core/sdk:3.1 dotnet watch run
```
...
```console
docker run --rm -it -p 8000:80 -v c:\git\dotnet-docker\samples\Tanyo.API:c:\app\ -w \app\Tanyo.API -e ASPNETCORE_URLS=http://+:80 --name Tanyo.APIsample mcr.microsoft.com/dotnet/core/sdk:3.1 dotnet watch run
```
